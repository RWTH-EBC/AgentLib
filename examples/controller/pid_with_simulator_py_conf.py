...
from agentlib.modules.communicator.local import LocalSubscriptionCommunicatorConfig
from agentlib.modules.controller.pid import PIDConfig
from agentlib.modules.simulator import SimulatorConfig
from agentlib.modules.utils.agent_logger import AgentLoggerConfig
...
pid_ag_config_py = AgentConfig(
    id="PID",
    modules=[
        AgentLoggerConfig(module_id="myLogger", filename="custom_file.log"),
        PIDConfig(
            module_id="pid",
            ub=500,
            lb=0,
            ...
            Kp=1000,
            setpoint=21,
            input=AgentVariable(name="u", value=0, alias="room_temp"),
            output=AgentVariable(name="y", value=0, alias="heating_power", shared=True),
        ),
        LocalSubscriptionCommunicatorConfig(
            module_id="myComm", subscriptions=["Process"]
        ),
    ],
)
process_agent_config_py = AgentConfig(
    id="Process",
    modules=[
        SimulatorConfig(
            module_id="sim",
        ...
        LocalSubscriptionCommunicatorConfig(module_id="comm", subscriptions=["PID"]),
        AgentLoggerConfig(module_id="myLogger", filename="custom_file_pid.log"),
    ],
)
...
def run_example(with_plots=True, log_level=logging.INFO):
    ...
    pid_agent = ag.Agent(config=pid_ag_config_py, env=env)
    process_agent = ag.Agent(config=process_agent_config_py, env=env)
    env.run(until=1000)
