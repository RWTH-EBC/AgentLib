...
MODEL_TYPES = plugin_import.SaveUpdateDict(**{
    'statespace': ModuleImport(
        import_path="agentlib.models.scipy_model",
        class_name="ScipyStateSpaceModel"
    ),
    'fmu': ModuleImport(
        import_path="agentlib.models.fmu_model",
        class_name="FmuModel"
    ),
})
...
def get_model_type(model_type) -> type:
    ...
    msg = f"Given model_type '{model_type}' is neither in the AgentLib nor in " \
          f"installed plugins. "
    if matches:
